...
class TestDATP(unittest.TestCase):
    ...
    def test_ioc_upload_record_found(self, mock_lookup):
        ...
        result = DATP.ioc_upload('testioc', 'Block', 'INC123')
    ...
    @patch('application.controllers.defender.DATP.ioc_type_finder', return_value='IpAddress')
    def test_ioc_upload_ipaddress(self, mock_type, mock_lookup):
        result = DATP.ioc_upload('8.8.8.8', 'Block', 'INC123')
        self.assertEqual(result, {'Added': False, 'IOC': '8.8.8.8', 'Platform': 'DATP', 'Action': 'None'})

    @patch('application.controllers.defender.DATP.lookup_record_to_db', return_value=None)
    @patch('application.controllers.defender.DATP.ioc_type_finder', return_value='DomainName')
    def test_ioc_upload_domain(self, mock_type, mock_lookup):
        result = DATP.ioc_upload('example.com', 'Block', 'INC123')
        self.assertEqual(result, {'Added': False, 'IOC': 'example.com', 'Platform': 'DATP', 'Action': 'None'})

    @patch('application.controllers.defender.DATP.lookup_record_to_db', return_value=None)
    @patch('application.controllers.defender.DATP.ioc_type_finder', return_value='FileSha256')
    ...
    def test_ioc_upload_file_success(self, mock_add, mock_upload, mock_type, mock_lookup):
        result = DATP.ioc_upload('a'*64, 'Block', 'INC123')
    ...
    @patch('application.controllers.defender.DATP.add_record_to_db')
    def test_ioc_upload_file_fail(self, mock_add, mock_upload, mock_type, mock_lookup):
        result = DATP.ioc_upload('b'*64, 'Block', 'INC123')
        self.assertEqual(result, {'Added': False, 'IOC': 'b'*64, 'Platform': 'DATP', 'Action': 'None'})
        mock_upload.assert_called_once()
        mock_add.assert_called_once()

    def test_ioc_type_finder(self):
        self.assertEqual(DATP.ioc_type_finder('a'*64), 'FileSha256')
        self.assertEqual(DATP.ioc_type_finder('b'*32), 'FileMd5')
        self.assertEqual(DATP.ioc_type_finder('c'*40), 'FileSha1')
        self.assertEqual(DATP.ioc_type_finder('8.8.8.8'), 'IpAddress')
        self.assertEqual(DATP.ioc_type_finder('example.com'), 'DomainName')
        self.assertEqual(DATP.ioc_type_finder('weirdinput'), 'DomainName')
    ...
    def test_add_record_to_db(self, mock_dynamo):
        ...
        DATP.add_record_to_db('ioc', 'FileSha256', 'Block', 'INC123')
