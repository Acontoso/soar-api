...
class Umbrella:
    ...
    def upload(cls, ioc: str, incident_id: str) -> bool:
        ...
        if access_token:
            ...
            for lst in UMBRELLA_DEST_LIST_IDS:
                ...
                response = requests.post(url=endpoint, data=json.dumps(pl), headers=headers)
    ...
    def get_cisco_token(self) -> str:
        ...
        try:
            ...
            response = requests.post(url=token_endpoint, data=data, headers=header, auth=auth)
            response.raise_for_status()
            token = response.json().get("access_token")
            current_app.logger.error(
                "[+] Umbrella access token returned"
            )
            return token
